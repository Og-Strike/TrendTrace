...
def generate_mask(input_image, net, palette, device = 'cpu',name='noname'):
    ...
    for cls in classes_to_save:
        ...
        alpha_mask_img.save(os.path.join(alpha_out_dir, f'{name}_{cls}.png'))

    # Save final cloth segmentations
    cloth_seg = Image.fromarray(output_arr[0].astype(np.uint8), mode='P')
    cloth_seg.putpalette(palette)
    cloth_seg = cloth_seg.resize(img_size, Image.BICUBIC)
    cloth_seg.save(os.path.join(cloth_seg_out_dir, f'{name}_final_seg.png'))
    return cloth_seg
...
def check_or_download_model(file_path):
    if not os.path.exists(file_path):
        ...
        print("Error Finding the model check curretn cwd")
...
def main(args):
    ...
    cwd = os.getcwd()
    model_path = os.path.join(cwd, 'model', 'cloth_segm.pth')
    ...
    cloth_seg = generate_mask(img, net=model, palette=palette, device=device,name=args.name_image)
